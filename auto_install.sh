...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --service-install) SERVICE_INSTALL="$2"; shift 2 ;;
            --slave-ip) SLAVE_IP="$2"; shift 2 ;;
            --slave-port) SLAVE_PORT="$2"; shift 2 ;;
            --check-interval) CHECK_INTERVAL="$2"; shift 2 ;;
            --dns-path) DNS_PATH="$2"; shift 2 ;;
            --listen-port) LISTEN_PORT="$2"; shift 2 ;;
            --auto-backup) AUTO_BACKUP="$2"; shift 2 ;;
            --secret-key) SECRET_KEY="$2"; shift 2 ;;
            --uninstall) UNINSTALL_TARGET="$2"; shift 2 ;;
...
# 检查系统要求，并根据版本设置正确的二进制文件名
check_system() {
    print_info "检查系统兼容性..."
    
    if [[ ! -f /etc/os-release ]]; then
        print_error "无法找到 /etc/os-release 文件，无法确定操作系统版本。"
        exit 1
    fi
    ...
    if [[ "$ID" == "debian" ]]; then
        case "$VERSION_ID" in
            "11")
                print_success "检测到 Debian 11 (Bullseye)。使用 Debian 11 兼容包。"
                MASTER_BINARY_NAME="master11"
                SLAVE_BINARY_NAME="slave11"
                ;;
            "12")
                print_success "检测到 Debian 12 (Bookworm)。使用 Debian 12 标准包。"
                MASTER_BINARY_NAME="master"
                SLAVE_BINARY_NAME="slave"
                ;;
            *)
                print_error "检测到不支持的 Debian 版本: $VERSION_ID ($VERSION_CODENAME)"
                print_error "此脚本仅支持 Debian 11 和 12。"
                exit 1
                ;;
        esac
    else
        print_error "检测到不支持的操作系统: $PRETTY_NAME"
        print_error "此脚本仅为 Debian 11 和 12 设计。"
        exit 1
    fi
...
configure_master() {
    ...
    if [ -n "$SLAVE_IP" ]; then
        slave_ip="$SLAVE_IP"; print_info "使用命令行参数: 被控端IP = $slave_ip"
    fi
    if [ -n "$SLAVE_PORT" ]; then
        slave_port="$SLAVE_PORT"; print_info "使用命令行参数: 被控端端口 = $slave_port"
    fi
    if [ -n "$CHECK_INTERVAL" ]; then
        check_interval="$CHECK_INTERVAL"; print_info "使用命令行参数: 检测间隔 = $check_interval"
    fi
    
    if [ -t 0 ] && [ -z "$SLAVE_IP" ]; then
...
configure_slave() {
    ...
    if [ -n "$SECRET_KEY" ]; then
        secret_key="$SECRET_KEY"; print_info "使用命令行参数: 通信密钥已提供"
    fi
    if [ -n "$DNS_PATH" ]; then
        dns_path="$DNS_PATH"; print_info "使用命令行参数: DNS路径 = $dns_path"
    fi
    if [ -n "$LISTEN_PORT" ]; then
        listen_port="$LISTEN_PORT"; print_info "使用命令行参数: 监听端口 = $listen_port"
    fi
    if [ -n "$AUTO_BACKUP" ]; then
        auto_backup="$AUTO_BACKUP"; print_info "使用命令行参数: 自动备份 = $auto_backup"
    fi

    if [ -t 0 ] && [ -z "$SECRET_KEY" ]; then
...
install_master() {
    ...
    if [ -d "$TEMP_DIR/master" ]; then
        ...
        chmod +x "/opt/ip_monitor_master/${MASTER_BINARY_NAME}"
    else
...
ExecStart=/opt/ip_monitor_master/${MASTER_BINARY_NAME}
Restart=always
RestartSec=10
[Install]
...
install_slave() {
    ...
    if [ -d "$TEMP_DIR/slave" ]; then
        ...
        chmod +x "/opt/ip_monitor_slave/${SLAVE_BINARY_NAME}"
    else
...
ExecStart=/opt/ip_monitor_slave/${SLAVE_BINARY_NAME}
Restart=always
RestartSec=10
[Install]
WantedBy=multi-user.target
...
# 配置防火墙
configure_firewall() {
    local port_to_open="${listen_port:-$DEFAULT_LISTEN_PORT}"
    print_info "检查防火墙状态..."
    if command -v ufw &> /dev/null && ufw status | grep -q 'Status: active'; then
        print_warning "检测到UFW防火墙，请手动开放端口: sudo ufw allow ${port_to_open}/tcp"
    elif command -v firewall-cmd &> /dev/null && firewall-cmd --state | grep -q 'running'; then
        print_warning "检测到firewalld防火墙，请手动开放端口: sudo firewall-cmd --permanent --add-port=${port_to_open}/tcp && sudo firewall-cmd --reload"
    fi
}
